...
def websocket_message(flow: websocket.WebSocketFlow):
    """
    Filters WebSocket messages for the specified target endpoint.
    """
    if TARGET_ENDPOINT in flow.request.url:
        for message in flow.messages:
            direction = "client_to_server" if message.from_client else "server_to_client"
            try:
                # Decode and identify message type
                content, content_type = detect_message_type(message.content.decode("utf-8"))
...
def done():
    """
    Ensures all traffic is saved when mitmproxy stops.
    """
    try:
        with open(TRAFFIC_FILE, "w") as f:
            json.dump(filtered_traffic, f, indent=4)
